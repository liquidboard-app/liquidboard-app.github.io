...
export const PageHeading = styled.header<{ $compact?: boolean; $tight?: boolean; $fullDescription?: boolean }>`
  ...
  p { max-width: ${({ $fullDescription }) => $fullDescription ? 'none' : '750px'}; margin: 20px auto 0; color: #000; font-size: clamp(17px, 1.35dvw, 20px); line-height: 1.58; font-weight: 540; letter-spacing: -.012em; }
...
export const GlassCard = styled.article`
  ...
  box-shadow: 0 20px 55px rgba(21, 21, 21, .09);
...
export const Tab = styled(NavLink)`
  ...
  &.active { border-color: #2c2724; background: #2c2724; color: #fff; }
